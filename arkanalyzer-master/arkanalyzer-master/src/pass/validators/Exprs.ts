...
export class AbsInvokeValidator extends ValueValidator<AbstractInvokeExpr> {
    private static readonly INSTANCE = new AbsInvokeValidator();
    ...
    checkArg(arg: Value, index: number, ctx: SummaryReporter): void {
        if (!((arg instanceof Local) || (arg instanceof Constant))) {
            ctx.error(`arg ${index} is not local or constant`);
        }
    }

    static {
        AbsInvokeValidator.register([AbstractInvokeExpr, (v: AbstractInvokeExpr, ctx: MethodCtx, mtd: ArkMethod): void => {
