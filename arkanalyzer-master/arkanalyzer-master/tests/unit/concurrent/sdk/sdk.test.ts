...
function buildScene(): Scene {
    ...
    config.buildFromProjectDir(path.join(__dirname, '../../../resources/concurrent/pta/sdk'));
...
describe('SdkTest', () => {
    ...
    it('case1: collections.Array.create()', () => {
        let target;
        for (let node of pag.getNodesIter()) {
            let pagNode = node as PagNode;
            let pagValue = pagNode?.getValue();
            if (pagValue && pagValue instanceof Local && pagValue.getName() === 'array') {
                target = pagNode;
            }
        }
    ...
    it('case2: ArkTSUtils.ASON.parse()', () => {
        let target;
        for (let node of pag.getNodesIter()) {
            let pagNode = node as PagNode;
            let pagValue = pagNode?.getValue();
            if (pagValue && pagValue instanceof Local && pagValue.getName() === 'obj') {
                target = pagNode;
            }
        }
        assert(target !== undefined);
    ...
    it('case3: UIUtils.makeObserved()', () => {
        let target;
        for (let node of pag.getNodesIter()) {
            let pagNode = node as PagNode;
            let pagValue = pagNode?.getValue();
            if (pagValue && pagValue instanceof Local && pagValue.getName() === 'obj2') {
                target = pagNode;
            }
        }
        assert(target !== undefined);
